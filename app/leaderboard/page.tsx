...
function TrophyBig() {
  return (
    <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
      ...
      <rect x="10" y="31" width="20" height="5" rx="2.5" fill="#ff4d6d"/>
    </svg>
  );
}
      ...
      <main style={{ padding: '24px 16px', maxWidth: 680, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          {tabs.map(({ key, label }) => (
            <button key={key} onClick={() => setTab(key)} style={{
              ...
              boxShadow: tab === key ? '0 3px 10px rgba(255,77,109,0.35)' : 'none',
            }}>{label}</button>
          ))}
        </div>
        ...
        {localBest && (
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 11, color: 'var(--accent)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: 0.5 }}>Your best</div>
              <div style={{ fontSize: 22, fontWeight: 900, color: 'var(--accent)' }}>{formatTime(localBest)}</div>
            </div>
            <MedalIcon rank={1}/>
          </div>
        )}
        ...
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
          <div style={{
            display: 'grid', gridTemplateColumns: '48px 1fr 100px 130px',
          ...
          {MOCK[tab].map((e, i) => (
            <div key={i} style={{
              ...
              onMouseEnter={el => el.currentTarget.style.background = 'rgba(255,77,109,0.05)'}
              onMouseLeave={el => el.currentTarget.style.background = 'transparent'}>
              <div style={{ display: 'flex', alignItems: 'center' }}><MedalIcon rank={e.rank}/></div>
              <div style={{ fontWeight: 600, color: 'var(--text)', fontSize: 14 }}>{e.name}</div>
              <div style={{ fontWeight: 800, color: 'var(--accent)', fontSize: 14 }}>{formatTime(e.time)}</div>
              <div style={{ fontSize: 13, color: 'var(--muted)' }}>{e.city}</div>
            </div>
          ))}
        </div>
        <p style={{ textAlign: 'center', marginTop: 12, fontSize: 12, color: 'var(--muted)' }}>Connect Supabase to show real scores</p>
      </main>
    </div>
  );
}
